...
def loadLocalFiles(path, variables=None, task=None):
    ...
    if task is None:
        ...
        if task == 1:
            ...
            df = df[df["AGE"]>=18]
            df.loc[(df['SEX']>=7)&(df['SEX']<=9), 'SEX'] = np.nan
            df.loc[df['SEX']>=900, 'SEX'] = np.nan
            df.loc[(df['RACEA']>=7)&(df['RACEA']<=9), 'RACEA'] = np.nan
            df.loc[df['RACEA']>=900, 'RACEA'] = np.nan
            df.loc[(df['HISPYN']>=7)&(df['HISPYN']<=9), 'HISPYN'] = np.nan
            df.loc[df['HISPYN']>=900, 'HISPYN'] = np.nan
            df.loc[(df['POVCAT']>=7)&(df['POVCAT']<=9), 'POVCAT'] = np.nan
            df.loc[df['POVCAT']>=900, 'POVCAT'] = np.nan
            df.loc[(df['EDUCYR']>=96)&(df['EDUCYR']<=99), 'EDUCYR'] = np.nan
            df = df.dropna(subset=selected_cols)
            df = df.reset_index(drop=True)
            
        elif task == 2:
            selected_cols = ['HINOTCOV', 'AGE', 'SEX', 'RACEA', 'EDUCYR', 'POVCAT', 'WORKEV']
            df = df[selected_cols]

            df = df[(df["AGE"]>=18) & (df["AGE"]<=64)]
            df.loc[(df['HINOTCOV']==9)|(df['HINOTCOV']==0), 'HINOTCOV'] = np.nan
            df.loc[(df['SEX']>=7)&(df['SEX']<=9), 'SEX'] = np.nan
            df.loc[df['SEX']>=900, 'SEX'] = np.nan
            df.loc[(df['RACEA']>=7)&(df['RACEA']<=9), 'RACEA'] = np.nan
            df.loc[df['RACEA']>=900, 'RACEA'] = np.nan
            ...
            selected_cols = ['DVTOTVIS', 'AGE', 'SEX', 'RACEA', 'POVCAT', 'HINOTCOV']
            df = df[selected_cols]
            ...
            selected_cols = ['ERTOTVIS', 'AGE', 'HEALTH', 'HINOTCOV', 'CHRONIC_COUNT']
            ...
            selected_cols = ['K6SUM', 'AGE', 'SEX', 'POVCAT', 'WORKEV', 'MARSTAT']
            df = df[selected_cols]
            
            df = df[df["AGE"]>=18]
            df.loc[(df['K6SUM']==96)|(df['K6SUM']==98), 'K6SUM'] = np.nan
            df.loc[(df['SEX']>=7)&(df['SEX']<=9), 'SEX'] = np.nan
            df.loc[df['SEX']>=900, 'SEX'] = np.nan
            df.loc[(df['POVCAT']>=7)&(df['POVCAT']<=9), 'POVCAT'] = np.nan
            df.loc[df['POVCAT']>=900, 'POVCAT'] = np.nan
            ...
            selected_cols = ['ADDAYA', 'AGE', 'SEX', 'HEALTH', 'CHRONIC_COUNT']
            df["CHRONIC_COUNT"] = create_chronic_count(df)
            df = df[selected_cols]
            
            df = df[df["AGE"]>=18]
            df.loc[(df['ADDAYA']>=6)&(df['ADDAYA']<=9), 'ADDAYA'] = np.nan
            df.loc[(df['SEX']>=7)&(df['SEX']<=9), 'SEX'] = np.nan
            df.loc[df['SEX']>=900, 'SEX'] = np.nan
            df = df.dropna(subset=selected_cols)
            df = df.reset_index(drop=True)
            ...
            selected_cols = ['AGE', 'SEX', 'RACEA', 'HEALTH', 'HYPERTENEV', 'CHOLHIGHEV', 'DIABETICEV_NEXT_YEAR', 'DIABETICEV']
            df = df.sort_values(by=['MEPSID', 'YEAR'])
            df['DIABETICEV_NEXT_YEAR'] = df.groupby('MEPSID')['DIABETICEV'].shift(-1)
            df = df[selected_cols]

            df = df[df["DIABETICEV"]==1]
            df.loc[(df['SEX']>=7)&(df['SEX']<=9), 'SEX'] = np.nan
            df.loc[df['SEX']>=900, 'SEX'] = np.nan
            df.loc[(df['RACEA']>=7)&(df['RACEA']<=9), 'RACEA'] = np.nan
            df.loc[df['RACEA']>=900, 'RACEA'] = np.nan
            ...
            selected_cols = ['POVCAT', 'AGE', 'SEX', 'RACEA', 'EDUCYR', 'FTOTVAL']
            df = df[selected_cols]

            df.loc[(df['POVCAT']>=7)&(df['POVCAT']<=9), 'POVCAT'] = np.nan
            df['FTOTVAL'] = df['FTOTVAL'].fillna(df['FTOTVAL'].median())
            df['FTOTVAL_Log'] = np.log1p(df['FTOTVAL'])
            df = df.dropna(subset=selected_cols)
            df = df.reset_index(drop=True)

        elif task == 9:
            selected_cols = ['CHRONIC_COUNT', 'AGE', 'SEX', 'RACEA', 'HEALTH']
            df["CHRONIC_COUNT"] = create_chronic_count(df)
            df = df[selected_cols]
            ...
            selected_cols = ['PHQ2', 'AGE', 'SEX', 'MARSTAT', 'K6SUM']
            df = df[selected_cols]

            df = df[df["AGE"]>=18]
            df.loc[(df['PHQ2']==96)|(df['PHQ2']==98), 'PHQ2'] = np.nan
            df['K6SUM'] = df['K6SUM'].fillna(df['K6SUM'].median())
            df = df.dropna(subset=selected_cols)
            df = df.reset_index(drop=True)
