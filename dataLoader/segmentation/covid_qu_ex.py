...
def getCovid_QU_EX(path):
    """
    Fetch Covid_QU_EX Dataset, process, and return in the following format:
        rawdata (list): A list of all extracted file paths.
    """
    kaggle_url = "anasmohammedtahir/covidqu"
    rawdata = datasetLoad(url=kaggle_url, path=path, datasetName="Covid_QU_EX")

    if not rawdata:
        raise ValueError("Failed to fetch or process the Covid_QU_EX Dataset.")
...
def datasetLoad(url, path, datasetName):
    ...
    try:
        if os.path.exists(datasetPath):
            ...
            print_sys("Downloading Pancreas CT Dataset...")
