...
def getIS_A(path, subtitle):
    url = "https://raw.githubusercontent.com/SotirisKot/Content-Aware-Node2Vec/master/datasets/relation_instances_edgelists/isa.edgelist"
    return datasetLoad(url=url, subtitle=subtitle, path=path, datasetName="IS-A")
...
def loadLocalFiles(dataSplitPath):
    ...
    trainset = {
        "source": train_pos_data + train_neg_data,
        "target": train_pos_label + train_neg_label
    }

    testset = {
        "source": test_pos_data + test_neg_data,
        "target": test_pos_label + test_neg_label
    }
