...
def getKvasir(path):
    """
    Fetch Kvasir data, process, and return in the following format:
        rawdata (list): A list of all extracted file paths.
    """
    urls = {
        "Kvasir_v2": "https://datasets.simula.no/downloads/kvasir/kvasir-dataset-v2.zip",
        "Kvasir_v3": "https://datasets.simula.no/downloads/kvasir/kvasir-dataset-v2-features.zip",
    }
...
def datasetLoad(urls, path, datasetName):
    """
    Downloads and extracts Kvasir dataset from multiple URLs.
...
def extractZipFile(zip_file, extract_path):
    ...
    try:
        ...
        print_sys(f"Extracted {zip_file}")
        os.remove(zip_file)
    except Exception as e:
