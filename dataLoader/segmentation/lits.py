...
def getLiTS(path):
    """
    Fetch LiTS Dataset, process, and return in the following format:
        rawdata (list): A list of all extracted file paths.
    """
    kaggle_url = "harshwardhanbhangale/lits-dataset"
    rawdata = datasetLoad(url=kaggle_url, path=path, datasetName="LiTS")

    if not rawdata:
        raise ValueError("Failed to fetch or process the LiTS Dataset.")
...
def datasetLoad(url, path, datasetName):
    ...
    try:
        if os.path.exists(datasetPath):
            ...
            print_sys("Downloading LiTS Dataset...")
